...
```{r, echo = TRUE, include = TRUE}
# get rid of all the NAs in the maternal mortality column
# group by country and arrange by year and grab the latest number for each country
df_country <- 
  filter(mm_gni_clean, is.na(maternal_mortality) == FALSE) %>%
  group_by(country) %>%
  arrange(year) %>%
  summarize(value = last(maternal_mortality))

# rename columns to choroplethr expected names
names(df_country) <- c("region", "value")

# make country names be all lower case
df_country$region <- tolower(df_country$region)

# fix some of the more blatant country naming errors
df_country[157,1] <- "united kingdom"
df_country[124,1] <- "russia"
df_country[17,1]  <- "bolivia"
df_country[72,1]  <- "iran"
df_country[163,1] <- "venezuela"
df_country[99,1]  <- "micronesia"

# this library loads the plyr library which messes up group_by and summarize
# do not load until absolutely needed
library(choroplethr)
country_choropleth(df_country, title="Maternal Mortality per 100,000 Live Births")
```